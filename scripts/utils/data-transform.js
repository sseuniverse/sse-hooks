 ...
 * Into this:
 *
 * import { useBoolean, useCounter, useInterval } from 'sse-hooks'
 */
export function transformImports(data) {
  ...
  if (imports.length > 0) {
    ...
    const newImportStatement = `import { ${importNames} } from 'sse-hooks'`;
...
export function replaceRelativePaths(data) {
  return data.replace(/\[([^\[]+)\]\((.*?)\)/g, (match, title, link) => {
    ...
    const newLink = `/hooks/${camelToKebabCase(hookName)}#${typeName}`;
