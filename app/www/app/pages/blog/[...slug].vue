...
const items = computed(() =>
  Object.entries(tree.value).map(([key, value]) => ({
    label: key,
    component: value,
  })),
);
  ...
  >
        ...
        <template v-for="author in page.authors" :key="author.name">
          ...
          >
            ...
            <div class="flex flex-col">
              ...
              <span
                class="text-xs text-muted group-hover:text-primary transition-colors"
    ...
    <template #right>
      <div>
        ...
        <nav
          class="h-full sticky top-(--ui-header-height) max-h-[calc(100vh-var(--ui-header-height))] hidden lg:block"
        >
          <ProseCodeTree
            v-if="activePath"
            ...
            :ui="{
              list: 'border-default',
              content:
                '[&>div>pre]:bg-muted/50 [&>div>pre]:border-default [&>div>pre]:rounded-none',
            }"
          />
          <div
            v-else
