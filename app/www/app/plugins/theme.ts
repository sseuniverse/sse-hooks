...
export default defineNuxtPlugin({
  ...
  setup() {
    ...
    if (import.meta.client) {
      const primary = localStorage.getItem("sse-ui-primary");
      if (primary) appConfig.ui.colors.primary = primary;

      const neutral = localStorage.getItem("sse-ui-neutral");
      if (neutral) appConfig.ui.colors.neutral = neutral;

      const icons = localStorage.getItem("sse-ui-icons");
      if (icons)
        appConfig.ui.icons = themeIcons[
          icons as keyof typeof themeIcons
        ] as any;
      ...
      restoreState("sse-ui-ai-theme");
      restoreState("sse-ui-custom-colors");
      restoreState("sse-ui-css-variables");

      try {
        const extras = JSON.parse(
          localStorage.getItem("sse-ui-ai-theme") || "{}",
        );
        if (extras.colors) {
    ...
    if (import.meta.server) {
      ...
      // The script IDs (chat-custom-colors, chat-css-variables, sse-ui-radius, etc.)
        ...
        script: [
          {
            innerHTML: `
            (function() {
              var primaryColor = localStorage.getItem('sse-ui-primary');
              var neutralColor = localStorage.getItem('sse-ui-neutral');
              ...
              var colorsEl = document.querySelector('style#sse-ui-colors');
              if (colorsEl) {
                swapColors(colorsEl);
              } else {
                requestAnimationFrame(function() {
                  var el = document.getElementById('sse-ui-colors');
                  if (el) swapColors(el);
                });
              }
            })();
          ...
          {
            innerHTML: `
            if (localStorage.getItem('sse-ui-radius')) {
              var el = document.querySelector('style#sse-ui-radius');
              if (el) { el.innerHTML = ':root { --ui-radius: ' + localStorage.getItem('sse-ui-radius') + 'rem; }'; }
            }
          `.replace(/\s+/g, " "),
            type: "text/javascript",
            tagPriority: -1,
          ...
          {
            innerHTML: `
            var bapEl = document.querySelector('style#sse-ui-black-as-primary');
            if (bapEl) {
              if (localStorage.getItem('sse-ui-black-as-primary') === 'true') {
          ...
          {
            innerHTML: [
              `if (localStorage.getItem('sse-ui-font')) {`,
              `var font = localStorage.getItem('sse-ui-font');`,
              `var fontEl = document.querySelector('style#sse-ui-font');`,
          ...
          },
          {
            innerHTML: `
            (function() {
              var raw = localStorage.getItem('sse-ui-custom-colors');
              if (raw) {
                try {
          ...
          },
          {
            innerHTML: `
            (function() {
              var raw = localStorage.getItem('sse-ui-css-variables');
              if (raw) {
                try {
